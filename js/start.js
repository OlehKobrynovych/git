...
// 
arr.indexOf(el, startIndex);   // шукає, el починаючи з індексу startIndex, і повертає індекс першого знайденого елемента, якщо не знайдено то поверне '-1' 
arr.lastIndexOf(el, startIndex);   // те саме, але шукає справа наліво. 
arr.includes(el, startIndex);      // шукає, el починаючи з індексу startIndex, повертає, true якщо знайдений і false якщо не знайде. 
...
// .......................... map()
arr.map(function(el, index, array) {....});    // map() Метод створює новий масив, заповнений результатами виклику наданої функції для кожного елемента в масиві
...
const regex = /[^\w\s]/ig;     // шукаємо будь-який символ, який не є символом слова чи пробілом (шукаємо крапку)
...
// let matches = /(asd)/.exec('qwe asd sdgs asd !');
...
// ......................find()
arr1.find((el, index, arr) => el > 10);     // Метод повертає значення першого елемента масиву , який проходить тестову функцію. 
...
//.....................every()
arr1.every((el, index, arr) => el < 40);     // Перевіряє масив, якщо всі значення масиву пройдуть тест то поверне true, якщо ні то поверне false 
...
//................................Object.assign()
Object.assign(obj, obj1, obj2, obj3);    // він копіює всі властивості, з одного або декількох об'єктів в об'єкт obj
...
Promise.all(promises)- очікує виконання всіх Проміс і повертає масив з результатами. Якщо будь-який із зазначених Проміс поверне помилку, то результатом роботи Promise.allбуде ця помилка, результати інших Проміс будуть ігноруватися.
Promise.allSettled(promises) (Розміщений недавно) - чекає, поки всі Проміс завершаться і повертає їх результати у вигляді масиву з об'єктами, у кожного об'єкта два властивості:
